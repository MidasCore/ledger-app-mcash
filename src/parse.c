...
tokenDefinition_t *getKnownToken(txContent_t *context) {
    ...
    for (i = 0; i < NUM_TOKENS_M20; i++) {
        currentToken = (tokenDefinition_t *) PIC(&TOKENS_M20[i]);
...
bool setContractType(uint8_t type, void *out) {
    switch (type) {
        ...
        case PROPOSALDELETECONTRACT:
            ...
            break;
        case STAKECONTRACT:
            os_memmove(out, "Stake\0", 6);
            break;
        case UNSTAKECONTRACT:
            os_memmove(out, "UnStake\0", 8);
            break;
        case WITNESSRESIGNCONTRACT:
            os_memmove(out, "Witness Resign\0", 15);
        case TRIGGERSMARTCONTRACT:
            os_memmove(out, "Trigger Smart Contract\0", 15);
            break;
        default:
            return false;
    };
...
void parseVariantTypeCount(txContext_t *context, uint8_t *data,
                           uint8_t *index, uint8_t dataLength, uint64_t *result, uint8_t type, uint8_t *count) {
    if (type != 0) {
        PRINTF("Error parseVariantTypeCount type %d!=0\n", type);
        THROW(0x6a80);
    }
    // get amount
    (*count) = parseVariant(context, data, index,
                            dataLength, result);
    (*count) += 1;
}
...
uint8_t parseTokenID(txContext_t *context, uint8_t *data, uint8_t *index,
    ...
    uint64_t assetId;
    uint8_t count;
    count = parseVariant(context, data, index, dataLength, &assetId);
    PRINTF("AssetId : %u\n", assetId);
    *outLength = CHAIN_CODE_LENGTH;
    snprintf((char *) out, MAX_TOKEN_LENGTH,
             "%d", (uint32_t) assetId);
    *outLength = strlen((const char *) out);
    return count;
...
//            if (data[*index]=='_'){
//                *outLength=3;
//                os_memmove(out,CHAIN_CODE,CHAIN_CODE_LENGTH);
//                (*index) += 1;
//                return 1;
//            }else THROW(0x6a80);
//        }
...
void parseTokenIDTypeCount(txContext_t *context, uint8_t *data, uint8_t *index,
                           uint8_t dataLength, uint8_t *out, uint8_t *outLength, uint8_t type, uint8_t *count) {
    if (type != 0) {
        PRINTF("Error parseTokenIDTypeCount type=%d !=0\n", type);
        THROW(0x6a80);
    }
    (*count) = parseTokenID(context, data, index, dataLength,
                            out, outLength);
    if (count > 0) {
        (*count) += 1;
    }
...
uint8_t parseAddress(txContext_t *context, uint8_t *data, uint8_t *index,
    ...
    out[ADDRESS_SIZE] = 0;
    (*index) += (ADDRESS_SIZE);
    PRINTF("Parse address %.*h \n", ADDRESS_SIZE, out);
    return ADDRESS_SIZE;
}

void parseAddressTypeCount(txContext_t *context, uint8_t *data, uint8_t *index,
                           uint8_t dataLength, uint8_t *out, uint8_t type, uint8_t *count) {
    if (type != 2) {
        PRINTF("Error parseAddressTypeCount type=%d != 2\n", type);
        THROW(0x6a80);
    }
    (*count) = parseAddress(context, data, index, dataLength, out);
    if ((*count) > 0) (*count) += 2;
}
            ...
            while (offset != length) {
                ...
                switch (context->stage) {
                    ...
                    case 1:
                        ...
                        switch (field) {
                            ...
                            case 2: // parameter
                                ...
                                if (offset > length) {
                                    ...
                                    if (!addToQueue(context, buffer + offset - count - 1, count - pending + 1))
                                        THROW(0x6a80);
                                    count = 0;
                                    offset = length;
                                    break;
                        ...
                        PRINTF("Stage end %d,%d,%d\n", context->stageQueue[0].total, context->stageQueue[0].count,
                               &count);
                    ...
                    case 2:
                        ...
                        switch (content->contractType) {
                            case TRANSFERCONTRACT: // Send MCASH
                                switch (field) {
                                    case 1: //owner_address
                                        parseAddressTypeCount(context, buffer, &offset, length,
                                                              content->account, type, &count);
                                        content->tokenNamesLength[0] = CHAIN_CODE_LENGTH;
                                        os_memmove(content->tokenNames[0], CHAIN_CODE, content->tokenNamesLength[0]);
                                        break;
                                    case 2: //to_address
                                        parseAddressTypeCount(context, buffer, &offset, length,
                                                              content->destination, type, &count);
                                        break;
                                    case 3: //amount
                                        parseVariantTypeCount(context, buffer, &offset,
                                                              length, &content->amount, type, &count);
                                        break;
                                    default:
                                        // INVALID
                                        THROW(0x6a80);
                            ...
                            case TRANSFERASSETCONTRACT:
                                switch (field) {
                                    case 1: // Token ID
                                        parseTokenIDTypeCount(context, buffer, &offset, length,
                            ...
                            case TRIGGERSMARTCONTRACT:
                                switch (field) {
                                    ...
                                    case 2: //contract_address
                                        parseAddressTypeCount(context, buffer, &offset, length,
                                                              content->contractAddress, type, &count);
                                        PRINTF("contract_address =  %.*h\n", ADDRESS_SIZE, content->contractAddress);
                                        break;
                                    case 3: //call_value
                                        parseVariantTypeCount(context, buffer, &offset,
                                                              length, &content->amount, type, &count);
                                    ...
                                    case 4: //data
                                        ...
                                        if (tmpNumber > 255 || (tmpNumber + offset) > length) {
                                            if (addToQueue(context, buffer + offset - count - 1,
                                                           length - offset + count + 1)) {
                                                count = 0;
                                                offset = length;
                                                break;
                                            } else THROW(0x6a80);
                                        ...
                                        // get selector
                                        PRINTF("Selector: %08x\n", U4BE(buffer, offset));
                                        if (os_memcmp(&buffer[offset], SELECTOR[0], 4) == 0)
                                            content->M20Method = 1; // check if transfer(address, uint256) function
                                        else if (os_memcmp(&buffer[offset], SELECTOR[1], 4) == 0)
                                            content->M20Method = 2; // check if approve(address, uint256) function
                                        else {
                                            if (!customContract) {
                                                // NOT ALLOWED
                                            ...
                                            if ((tmpNumber - 4) % 32 != 0) {
                                                PRINTF("length is not divided by 32bytes\n");
                                                THROW(0x6a80);
                                            }
                                            content->M20Method = 0;
                                        ...
                                        // check if DATA field size matchs M20 Transfer/Approve
                                        if (tmpNumber != M20_DATA_FIELD_SIZE) {
                                            THROW(0x6a80);
                                            PRINTF("DATA field size not matchs M20 Transfer/Approve\n");
                                        }
                                        // TO Address
                                        ...
                                        // Amount
                                        os_memmove(content->M20Amount, buffer + offset + 36, 32);
//                                        TODO check token M20
//                                        tokenDefinition_t* M20 = getKnownToken(content);
//                                        if (M20 == NULL) {
//                                            PRINTF("UnKnown token M20\n");
//                                            THROW(0x6a80);
//                                        }
//                                        content->decimals[0] = M20->decimals;
//                                        content->tokenNamesLength[0] = ADDRESS_SIZE;
//                                        os_memmove(content->tokenNames[0], content->contractAddress,
//                                                   content->contractAddress[ADDRESS_SIZE - 1]);
                                        offset += (uint8_t)(tmpNumber & 0xFF);
                                        count += (uint8_t)(tmpNumber & 0xFF) + 1;
                                        break;
                                    ...
                                    case 5: //call_token_value
                                        parseVariantTypeCount(context, buffer, &offset,
                                                              length, &content->amount2, type, &count);
                                        break;
                                    case 6: //token_id
                                        parseTokenIDTypeCount(context, buffer, &offset, length, content->tokenNames[0],
                                                              &content->tokenNamesLength[0], type,
                                                              &count);
//                                        if (type!=0) THROW(0x6a80);
//                                        // get token id
//                                        count = parseVariant(context, buffer, &offset,
...
//                                        content->tokenNamesLength[0] = strlen((const char *)content->tokenNames[0]);
                            ...
                            case EXCHANGECREATECONTRACT:
                                switch (field) {
                                    ...
                                    case 3: //First Token Amount
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->amount,
                                                              type,
                                                              &count);
                                        break;
                                    case 4: //First Token
                                    ...
                                    case 5: //First Token Amount
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->amount2,
                                                              type,
                                                              &count);
                                        break;
                                    default:
                            ...
                            case EXCHANGEWITHDRAWCONTRACT:
                                switch (field) {
                                    case 1: //owner_address
                                        ...
                                        break;
                                    case 2: //Exchange id
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->exchangeID,
                                                              type,
                                                              &count);
                                        break;
                                    case 3: //Token ID
                                        parseTokenIDTypeCount(context, buffer, &offset, length,
                                                              ...
                                                              type, &count);
                                    case 4: //Token Amount
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->amount, type,
                                                              &count);
                                        break;
                                    default:
                                        // INVALID
                            ...
                            case EXCHANGETRANSACTIONCONTRACT:
                                switch (field) {
                                    ...
                                    case 5: //Expected Amount
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->amount2, type,
                                                              &count);
                                        break;
                                    default:
                                        // INVALID
                                ...
                                break;
                            case STAKECONTRACT: // Stake Contract
                                switch (field) {
                                    case 1: //owner_address
                                        parseAddressTypeCount(context, buffer, &offset, length, content->account, type,
                                                              &count);
                                        break;
                                    case 2: //amount
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->amount, type,
                                        ...
                                        break;
                                    case 3: //duration
                                        parseVariantTypeCount(context, buffer, &offset, length, &content->duration,
                                                              type, &count);
                                        break;
                                    default:
                                        // INVALID
                            ...
                            case VOTEWITNESSCONTRACT: // Vote Witness
                                ...
                                break;
                            case UNFREEZEASSETCONTRACT:
                            case UNSTAKECONTRACT: // Vote Witness
                                switch (field) {
                                    case 1: //owner_address
                                        parseAddressTypeCount(context, buffer, &offset, length, content->account, type,
                                                              &count);
                            ...
                            case PROPOSALDELETECONTRACT: // Proposal Delete Contract
                                switch (field) {
                                    ...
                                    case 2: //Proposal id
                                        parseVariantTypeCount(context, buffer, &offset,
                                                              length, &content->exchangeID, type, &count);
                                        break;
                                    default:
                                        // INVALID
                                        THROW(0x6a80);
