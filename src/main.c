...
volatile char M20Action[9];
volatile char M20ActionSendAllow[8];
volatile char fullHash[HASH_SIZE*2+1];
volatile char exchangeContractDetail[50];

static const char const SIGN_MAGIC[] = "\x19MCASH Signed Message:\n";
...
void ui_approval_custom_contract_blue_init(void) {
    ...
    ui_approval_blue_cancel =
        (bagl_element_callback_t)io_seproxyhal_touch_tx_cancel;
    ui_approval_blue_values[0] = (const char*)M20Action;
    ui_approval_blue_values[1] = (const char*)fullContract;
    ui_approval_blue_values[2] = (const char*)toAddress;
    ui_approval_blue_values[3] = (const char*)G_io_apdu_buffer;
    ui_approval_blue_values[4] = (const char*)fromAddress;
...
//                }
    ...
    PRINTF("INFO contractType=%d hash=%.*h\n", txContent.contractType, HASH_SIZE, transactionContext.hash);

    getBase58FromAddres(txContent.account, (void *)fromAddress, &sha2);
            fromAddress[BASE58CHECK_ADDRESS_SIZE]='\0';    

    switch (txContent.contractType){
        case TRANSFERCONTRACT: // MCASH Transfer
        case TRANSFERASSETCONTRACT: // M1 Transfer
        case TRIGGERSMARTCONTRACT: // M20 Transfer
            
            os_memmove((void *)M20ActionSendAllow, "Send To\0", 8);
            if (txContent.contractType==TRIGGERSMARTCONTRACT){
                if (txContent.M20Method==1)
                    os_memmove((void *)M20Action, "Asset\0", 6);
                else if (txContent.M20Method==2){
                    os_memmove((void *)M20ActionSendAllow, "Allow\0", 8);
                    os_memmove((void *)M20Action, "Approve\0", 8);
                }else {
                    ...
                    snprintf((char *)M20Action, sizeof(M20Action), "%08x", txContent.customSelector);
                    ...
                    if (txContent.amount>0) {
                        os_memmove((void *)toAddress, CHAIN_CODE, CHAIN_CODE_LENGTH);
                ...
                convertUint256BE(txContent.M20Amount, 32, &uint256);
        ...
        case EXCHANGECREATECONTRACT:
            ...
            print_amount(txContent.amount,(void *)G_io_apdu_buffer,100, (strncmp((const char *)txContent.tokenNames[0], "MCASH", 3)==0)?SUN_DIG:txContent.decimals[0]);
            print_amount(txContent.amount2,(void *)G_io_apdu_buffer+100,100, (strncmp((const char *)txContent.tokenNames[1], "MCASH", 3)==0)?SUN_DIG:txContent.decimals[1]);
